...
player_weekly <- fromJSON("https://fantasy.premierleague.com/drf/element-summary/1")$fixtures_summary[1,]
...
for (i in player_details$id) {
  ...
  dftemp = fromJSON(json_url)$fixtures_summary
  if(!is.null(dim(dftemp))){
    dftemp = dftemp[1,]          # fixtures summary show next 3 games. we only need next immediate
...
write.csv(player_weekly, file="./Data/weekly_04fixture.csv", row.names=FALSE)
