...
# This is to get current season's player ID
...
player_hist  = cbind(player_id=1, player_names, player_hist[nrow(player_hist),])
...
for (i in player_details$id) {
  ...
  dftemp   = fromJSON(json_url)$history_past %>% tail(1)  # take only the previous season
...
## Subset to only latest season, which is 12, which is 2017/18
player_hist = player_hist[player_hist$season==12,]

## Save the historical data frame into a csv for easy reference later
write.csv(player_hist, file="./Data/player_history_lastseason.csv", row.names=FALSE)
