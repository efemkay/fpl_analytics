...
player_weekly = fromJSON("https://fantasy.premierleague.com/drf/element-summary/1")$history
...
for (i in player_details$id) {
  ...
  dftemp = fromJSON(json_url)$history
  # if(nrow(dftemp)==1){dftemp=NULL} else{dftemp=dftemp}
  if(!is.null(dim(dftemp))){
    ...
    player_weekly = rbind(player_weekly
                          , cbind(player_id=i, player_names, dftemp, row.names=NULL)
                          )
  }
}

## Save the historical data frame into a csv for easy reference later
write.csv(player_weekly, file="./Data/weekly_03history_currweekseas.csv", row.names=FALSE)
